...
class trainer1():
    def __init__(self, in_dim, seq_length, num_nodes, nhid , CL,l,dropout, lrate, wdecay, device, supports,decay):
...
class trainer7():
    def __init__(self, in_dim,in_dim_cluster, seq_length, num_nodes, cluster_nodes,CL, nhid , dropout, lrate, wdecay, device, supports,supports_cluster,transmit,decay):
        self.model = COHGCN(device, num_nodes,cluster_nodes, CL,dropout, supports=supports, supports_cluster=supports_cluster,
                           in_dim=in_dim,in_dim_cluster=in_dim_cluster, out_dim=seq_length, transmit=transmit,
                           residual_channels=nhid, dilation_channels=nhid, 
                           skip_channels=nhid * 8, end_channels=nhid * 16)
        self.model.to(device)
    ...
    def train(self, input, input_cluster, real_val,real_val_cluster):
        ...
        self.optimizer.zero_grad()
        #input = nn.functional.pad(input,(1,0,0,0))
        output,Siam_loss,_ = self.model(input,input_cluster)
        output = output.transpose(1,3)
        #output_cluster = output_cluster.transpose(1,3)
        #output = [batch_size,1,num_nodes,12]
        real = torch.unsqueeze(real_val,dim=1)
